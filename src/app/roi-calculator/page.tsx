...
import { Calculator, TrendingUp, Clock, DollarSign, ShieldCheck, ChevronDown, ChevronUp, ArrowRight } from "lucide-react";
...
export default function ROICalculatorPage() {
  const [cranes, setCranes] = useState(25);
  ...
  const hoursSaved = cranes * hours * 0.6 * inspectors;
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return;
    const leads = JSON.parse(localStorage.getItem("cranecheck_roi_leads") || "[]");
    leads.push({ email, cranes, hours, rate, inspectors, annualSavings, roi, ts: new Date().toISOString() });
    localStorage.setItem("cranecheck_roi_leads", JSON.stringify(leads));
    setSubmitted(true);
  }, [email, cranes, hours, rate, inspectors, annualSavings, roi]);
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "ROI Calculator", item: "https://cranecheck.com/roi-calculator" },
    ],
  };
        ...
        <section className="max-w-6xl mx-auto px-4 pt-16 pb-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
              Calculate Your <span className="text-amber-400">Inspection Savings</span>
            </h1>
            <p className="text-gray-400 text-lg max-w-2xl mx-auto">
              See how much time and money CraneCheck saves your team by replacing paper inspection forms with digital workflows.
            </p>
          </motion.div>
        </section>
        ...
        <section className="max-w-6xl mx-auto px-4 pb-20">
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
              ...
              <Slider label="Cranes inspected per month" min={5} max={200} step={1} value={cranes} onChange={setCranes} />
              <Slider label="Hours per inspection (paper forms)" min={1} max={8} step={0.5} value={hours} onChange={setHours} suffix=" hrs" />
              <Slider label="Average inspector hourly rate" min={25} max={75} step={1} value={rate} onChange={setRate} prefix="$" />
              <Slider label="Number of inspectors on staff" min={1} max={20} step={1} value={inspectors} onChange={setInspectors} />
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
              ...
              <h2 className="text-xl font-semibold text-white mb-6 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-amber-400" /> Your Savings with CraneCheck
              </h2>
              ...
              <div className="bg-[#0b1120] rounded-xl p-4 border border-gray-800 mb-6">
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div>
                    <p className="text-xs text-gray-500 uppercase mb-2">Paper Forms</p>
                    <p className="text-lg font-bold text-red-400"><AnimatedNumber value={Math.round(cranes * hours * inspectors)} suffix=" hrs/mo" /></p>
                    <p className="text-xs text-gray-500 mt-1">Manual tracking</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase mb-2">With CraneCheck</p>
                    <p className="text-lg font-bold text-emerald-400"><AnimatedNumber value={Math.round(cranes * hours * 0.4 * inspectors)} suffix=" hrs/mo" /></p>
                    <p className="text-xs text-gray-500 mt-1">Digital workflows</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 mb-6">
                ...
                <p className="text-sm text-amber-300">98% fewer missed inspection items</p>
              </div>
