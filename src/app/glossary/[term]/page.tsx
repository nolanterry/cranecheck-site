...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!entry) return {};
  return {
    title: `${entry.term} — Crane Inspection Glossary`,
    description: entry.definition,
    openGraph: {
      title: `${entry.term} — CraneCheck Glossary`,
      description: entry.definition,
    },
  ...
  const structuredData = [
    {
      ...
      name: entry.term,
      description: entry.definition,
      url: `https://cranecheck.co/glossary/${entry.slug}`,
      inDefinedTermSet: {
        "@type": "DefinedTermSet",
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "Glossary", item: "https://cranecheck.co/glossary" },
        { "@type": "ListItem", position: 3, name: entry.term, item: `https://cranecheck.co/glossary/${entry.slug}` },
      ],
    },
  ];
    ...
    <>
      ...
      <main id="main-content">
        ...
        <article className="max-w-4xl mx-auto px-4 py-12">
          <Link
            href="/glossary"
            className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-brand mb-6 transition-colors"
          >
            <ArrowLeft size={14} /> Back to Glossary
          </Link>

          <div className="flex items-start gap-3 mb-2">
            <span className="px-2.5 py-1 bg-brand/10 text-brand rounded text-xs font-medium">
              {entry.category}
            </span>
          </div>
          ...
          <p className="text-xl text-gray-600 leading-relaxed mb-8 border-l-4 border-brand pl-4">
            {entry.definition}
          </p>
                  ...
                  (rt) =>
                    rt && (
                      <Link
                        ...
                        className="p-4 rounded-lg border border-gray-200 hover:border-brand hover:shadow-sm transition-all group"
                      >
